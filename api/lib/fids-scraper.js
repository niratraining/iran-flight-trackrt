...
const STATUS_MAP = [
  // «باطل شد» هم عبارت رایج فرودگاه برای کنسلی‌ست (علاوه بر «لغو»)؛ قبلاً
  // فقط «لغو» چک می‌شد و پروازهای «باطل شد» هیچ‌وقت cancelled نمی‌شدن —
  // هم در داشبورد وضعیت غلط نشون داده می‌شد، هم چون از محاسبه‌ی نرخ
  // به‌موقع‌بودن کنار گذاشته نمی‌شدن، آمار ایرلاین‌ها رو هم آلوده می‌کرد.
  [/لغو|باطل/, 'cancelled'],
  [/نشست|فرود/, 'landed'],
  [/تاخیر|تأخیر/, 'delayed'],
  [/دریافت بار|نقاله/, 'landed'],
  // نکته: «پایان پذیرش» باید قبل از الگوی عمومی‌تر «پذیرش» چک بشه، وگرنه
  // چون «پایان پذیرش مسافر» خودش شامل کلمه‌ی «پذیرش» هم هست، همیشه با
  // الگوی پایین‌تر (checkin) مچ می‌شد و gate_closed هیچ‌وقت اجرا نمی‌شد —
  // نتیجه‌ش این بود که پروازهایی که پذیرششون تموم شده بود، برای همیشه
  // در داشبورد «در حال پذیرش» می‌موندن.
